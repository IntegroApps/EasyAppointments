...
App.Http.Booking = (function () {
    ...
    async function registerAppointment() {
        ...
        const respMP = await registerIdentifier(respRegister.service,respRegister.quantity,respRegister.price, respRegister.appointment_hash);
        console.log({respMP})

        const linkMP = document.createElement('a');
        linkMP.href = respMP.sandbox_init_point;

        linkMP.textContent = 'Pagar con MercadoPago';
    ...
    async function registerIdentifier(service_title, quantity, unit_price,hash_appointment) {


        const url = App.Utils.Url.siteUrl('booking/mercadoPago');

        return await $.ajax({
            url: url,
            method: 'post',
            data: { service_title, quantity: parseInt(quantity), unit_price: parseFloat(unit_price),hash_appointment },
            dataType: 'json',
        })
            .then(respuesta => {
                return respuesta;
